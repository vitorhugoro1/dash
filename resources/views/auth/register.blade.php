        ...
        <div class="rounded shadow">
            ...
            <div class="bg-white p-3 rounded-b">
                <form class="form-horizontal" method="POST" action="{{ route('register') }}">
                    ...
                    <div class="flex items-stretch mb-3">
                        <label for="email" class="text-right font-semibold text-grey-dark text-sm pt-2 pr-3 align-middle w-1/4">MAL Username</label>
                        <div class="flex flex-col w-3/4">
                            <input id="mal_username" type="text" class="flex-grow h-8 px-2 border rounded {{ $errors->has('mal_username') ? 'border-red-dark' : 'border-grey-light' }}" name="mal_username" value="{{ old('mal_username') }}" required>
                            {!! $errors->first('mal_username', '<span class="text-red-dark text-sm mt-2">:message</span>') !!}
                        </div>
                    </div>
